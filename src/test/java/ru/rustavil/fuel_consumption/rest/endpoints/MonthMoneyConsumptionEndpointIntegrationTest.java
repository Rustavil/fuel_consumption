...
public class MonthMoneyConsumptionEndpointIntegrationTest {
    ...
    @Test
    public void whenGetMonthMoneyConsumptionReportThenExpected() throws Exception {
        mvc.perform(
                get("/fuel_consumption/month_money")).
                andExpect(status().is2xxSuccessful()).
                andExpect(jsonPath("$", hasSize(2))).
                andExpect(jsonPath("$.*.money", hasItem(12000.00))).
                andExpect(jsonPath("$.*.money", hasItem(8000.00))).
    ...
    @Test
    public void whenGetMonthMoneyConsumptionReportForDriverThenExpected() throws Exception {
        mvc.perform(
                get("/fuel_consumption/month_money/"+driverDtoList.get(0).getId())).
                andExpect(status().is2xxSuccessful()).
                andExpect(jsonPath("$", hasSize(2))).
                andExpect(jsonPath("$.*.money", hasItem(4000.00))).
                andExpect(jsonPath("$.*.money", hasItem(6000.00))).
                andExpect(jsonPath("$.*.date", hasItem("2019-06-01"))).
                andExpect(jsonPath("$.*.date", hasItem("2019-07-01"))).
                andDo(r->System.out.println(r.getResponse().getContentAsString()));
    }
    ...
    @Test
    public void whenGetMonthMoneyConsumptionReportOnlyOneItemThenExpected() throws Exception {
        mvc.perform(
                get("/fuel_consumption/month_money").param("page", "0").param("size", "1")).
                andExpect(status().is2xxSuccessful()).
