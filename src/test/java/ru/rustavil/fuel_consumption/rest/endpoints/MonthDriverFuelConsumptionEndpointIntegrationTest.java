...
public class MonthDriverFuelConsumptionEndpointIntegrationTest {
    ...
    @Autowired
    private MonthDriverFuelConsumptionReport report;
    ...
    @Test
    public void whenLoadMonthDriverFuelOn2019_07ThenExpected() throws Exception {
        mvc.perform(
                get("/fuel_consumption/month_driver_fuel/2019_07_01")).
                andExpect(status().is2xxSuccessful()).
                andExpect(jsonPath("$", hasSize(2))).
                andExpect(jsonPath("$.*.driverIdentifier", hasItem(111111))).
                andExpect(jsonPath("$.*.driverIdentifier", hasItem(222222))).
                andExpect(jsonPath("$.*.fuelVolume", hasItem(100.5))).
                andExpect(jsonPath("$.*.fuelVolume", hasItem(401.0))).
                andExpect(jsonPath("$.*.fuelPrice", hasItem(2000.00))).
                andExpect(jsonPath("$.*.fuelPrice", hasItem(6000.00))).
                andExpect(jsonPath("$.*.fuelType", hasItem("TYPE_D"))).
                andExpect(jsonPath("$.*.date", hasItem("2019-07-01")));
    }

    @Test
    public void whenLoadMonthDriverFuelOn2019_07ForDriverThenExpected() throws Exception {
        mvc.perform(
                get("/fuel_consumption/month_driver_fuel/2019_06_01/"+driverDtoList.get(0).getId())).
                andExpect(status().is2xxSuccessful()).
                andExpect(jsonPath("$", hasSize(2))).
                andExpect(jsonPath("$.*.driverIdentifier", hasItem(111111))).
                andExpect(jsonPath("$.*.fuelVolume", hasItem(100.5))).
    ...
    @Test
    public void whenLoadMonthDriverFuelOn2019_07AndOneItemThenExpected() throws Exception {
        mvc.perform(
                get("/fuel_consumption/month_driver_fuel/2019_07_01").param("page", "0").param("size", "1")).
                andExpect(status().is2xxSuccessful()).
